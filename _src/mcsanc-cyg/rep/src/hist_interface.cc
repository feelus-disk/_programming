...
extern "C" {
  ...
  int set_3bh_(const char * hname0, const char * hname1, const char * hname2,
...
vector<ShmHist3d*> vs3h;
vector<ShmHist3d*> vs3hcum;
vector<ShmHist3d*> vs3hborn;
    ...
    const double *p3, const double *p4, const double *p5)
{
  ...
  //sem_wait(mcsanc_hist_fill_mutex);//moved into ShmHist
  for ( int ih=0; ih<vsh.size(); ih++){
    if ( hval_.hist_val[ih] != hval_.hist_val[ih] ) continue;
    vsh.at(ih)->Fill(hval_.hist_val[ih], *val, *wgt);
  }
  for ( int ih=0; ih<vs3h.size(); ih++)
    vs3h.at(ih)->Fill(hval_.hist_val, *val, *wgt);
...
int read_hist_(const int *restored)
{
  ...
  vector<ShmHist3d*> ::iterator its3h = vs3h.begin();
  for (; its3h!=vs3h.end(); its3h++){
    (*its3h)->ReadHistFromFile(fin, *restored);
    if ( 1 != *restored ) (*its3h)->BinQuadVegas();
  }
...
int read_born_hist_(const int *restored)
{
  ...
  vector<ShmHist3d*> ::iterator its3h = vs3hborn.begin();
  for (; its3h!=vs3hborn.end(); its3h++){
    (*its3h)->ReadHistFromFile(fin, *restored);
    if ( 1 != *restored ) (*its3h)->BinQuadVegas();
  }
...
/**
 * Sets variable bin 3d histograms parameters.
 * This routine is called upon initialization whil reading input.cfg file
 */
int set_3bh_(const char * hname0, const char * hname1, const char * hname2,
	int flag, int n0bins, int n1bins, int n2bins,
	double *bins0, double *bins1, double *bins2)
{
  vs3h.push_back(new ShmHist3d(hname0, hname1, hname2,
		flag, n0bins, n1bins, n2bins, bins0, bins1, bins2));
  vs3hcum.push_back(new ShmHist3d(hname0, hname1, hname2,
		flag, n0bins, n1bins, n2bins, bins0, bins1, bins2));
  vs3hborn.push_back(new ShmHist3d(hname0, hname1, hname2,
		flag, n0bins, n1bins, n2bins, bins0, bins1, bins2));
}
...
int init_shmhist_()
{
  ...
  for ( int ih = 0; ih < vs3h.size(); ih++){
    vs3h.at(ih)->InitShmHist();
    vs3hcum.at(ih)->InitShmHist();
    vs3hborn.at(ih)->InitShmHist();
  }
