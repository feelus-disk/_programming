...
C-----------------------------------------------------------------
C
C>    check if products fit the cuts
C
C-----------------------------------------------------------------
      ...
      double precision miv34b, mtr34b, xm34b, xp3b
      double precision pt3b, pt4b
      double precision eta3b, eta4b, eta5b
      double precision pt34b, eta34b
      double precision p3p4b

      double precision miv34c, mtr34c, xm34c, xp3c
      ...
      double precision phistarb, costetstarb, sintetstarb
      double precision phistarc, costetstarc, sintetstarc
...
c     Xm34
      if ( charge_id .eq. 0 ) xm34b = mtr34b/mz
      if ( charge_id .ne. 0 ) xm34b = mtr34b/mw
c     Xp3
      if ( charge_id .eq. 0 ) xp3b = pt3b/mz
      if ( charge_id .ne. 0 ) xp3b = pt3b/mw
c     phistarb
...
c        if ((fspart_id .eq. idel) .or. (fspart_id .eq. idmu)) then
...
c     34 transverse mass
         ...
         !if (dabs(cosphi34).gt.1d0) return
...
c     Xm34
         if ( charge_id .eq. 0 ) xm34c = mtr34c/mz
         if ( charge_id .ne. 0 ) xm34c = mtr34c/mw
c     Xp3
         if ( charge_id .eq. 0 ) xp3c = pt3c/mz
         if ( charge_id .ne. 0 ) xp3c = pt3c/mw
c     phistarc
...
c     apply calo cuts
     ...
     .          ( eta3c .gt. max_eta4 .or. eta4c .gt. max_eta3 )) goto 101
         ...
         if (comp_id .eq. iewhard) then
            if (fspart_id .eq. idmu) then
               if (p5(4).ge.2d0.and.rl35.le.0.1d0) goto 101
               if (charge_id.eq.0) then
                  if (p5(4).ge.2d0.and.rl45.le.0.1d0) goto 101
               endif
            endif
            if (p5(4).ge.p3(4)/10d0.and.rl35.gt.0.1d0.and.rl35.le.0.4d0) goto 101
            if (charge_id.eq.0) then
               if (p5(4).ge.p4(4)/10d0.and.rl45.gt.0.1d0.and.rl45.le.0.4d0) goto 101
            endif
         endif
...
C-----------------------------------------------------------------
C
C>    Print cuts that are turned on
C
C-----------------------------------------------------------------
