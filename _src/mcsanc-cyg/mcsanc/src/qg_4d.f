...
C     Vegas calls for 4d function
      ...
      if ( vg_exists .neqv. .true. ) then
        ...
        print *,
        ixplore = 1
        call llvegas(ndim, 1, qg_4d, limits,
...
!<-------------------------------------------------------------
      integer function qg_4d(ndim, arg, ncomp, f, limits, vwgt, iter)
      ...
      double precision aaborn(4), aborn(2), asoft(2), scalef, scaler, alphasPDF
...
C     PDF convolution
      ...
      call pdf_conv_subt_gl (x1,x2,scalef,subip,apdf_dconv)
...
C     Parton level cross sections
      ...
      if (charge_id .ne. 0) then
         if ( fspart_id .eq. idtopt ) then
            call sanc_br (-s,-t,-u,aborn,asoft)
            aaborn(1) = aborn(1)
            aaborn(2) = aborn(2)
            call sanc_br (-s,-u,-t,aborn,asoft)
            aaborn(3) = aborn(1)
            aaborn(4) = aborn(2)
         else
            call sanc_br (-s,-t,-u,aborn,asoft)
      ...
      do in=1,4
         f(1) = f(1) + 3 * aaborn(in) * apdf_dconv(in) * jac
      enddo
      iborn = iborn_

      !print *, "alphas=",alphas
c$$$      print *, "sqs,y,costh,subip",sqs,y,costh,subip
c$$$      print *, "x1,x2",x1,x2
c$$$      print *, "s,t,u",s,t,u
c$$$      print *, "mf,mf1",mf,mf1
c$$$      print *, "born",aaborn
c$$$      print *, "pdf",apdf_dconv
c$$$      stop
