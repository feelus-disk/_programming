...
************************************************************************
* File (cc_br_1322_2114.f) is created on Tue Aug  9 22:59:53 MSD 2011.
************************************************************************
* This is the FORTRAN module (cc_bo_1322_2114) to calculate EW
* integrated Born cross section for the anti-up + anti-bt -> anti-tp + anti-dn process.
*
...
* E-mail: <sanc@jinr.ru>
************************************************************************
      subroutine cc_bo_1322_2114 (s,born)
      implicit none!
...
************************************************************************
* This is the FORTRAN module (cc_br_1322_2114) to calculate Born
...
* E-mail: <sanc@jinr.ru>
************************************************************************
      subroutine cc_br_1322_2114 (qs,ts,us,born,soft,hard)
      implicit none!
      include 's2n_declare.h'
     ...
     & +log(1-mbt**2/s)*born/pi*alpha*qup**2*(1)
     & +log(1-mbt**2/s)**2*born/pi*alpha*qup*qbt*(7d0/4)
     & +log(1d0/mup**2*s)*born/pi*alpha*qup**2*(3d0/4)
     & +log(1d0/mup**2*s)*log(1-mbt**2/s)*born/pi*alpha*qup*qbt*(1)
     & +log(1d0/mbt**2*s)*born/pi*alpha*qbt**2*(3d0/4)
     & +log(1d0/mbt**2*s)*born/pi*alpha*qbt**2*mbt**2*(1d0/(s-mbt**2))
     & +log(1d0/mbt**2*s)*log(1-mbt**2/s)*born/pi*alpha*qup*qbt*(1d0/2)
     & +log(4*omega**2/s)*born/pi*alpha*qbt**2*(-1d0/2)
     & +log(4*omega**2/s)*born/pi*alpha*qup**2*(-1d0/2)
     & +log(4*omega**2/s)*log(1-mbt**2/s)*born/pi*alpha*qup*qbt*(-1)
     & +log(4*omega**2/s)*log(1d0/mup**2*s)*born/pi*alpha*qup*qbt*(-1d0/
     & 2)
     & +log(4*omega**2/s)*log(1d0/mbt**2*s)*born/pi*alpha*qup*qbt*(-1d0/
     & 2)
     & +ddilog(1-mbt**2/s)*born/pi*alpha*qup*qbt*(1d0/2)
     & +ddilog(-1d0/(s-mbt**2)*mbt**2)*born/pi*alpha*qup*qbt*(1d0/2)
     & +ddilog(1/(s-mbt**2)*s)*born/pi*alpha*qup*qbt*(-1)
     & +ddilog(1d0)*born/pi*alpha*qup*qbt*(-3d0/2)

      softvirtfsr=
     & +born/pi*alpha*qtp**2*(3d0/4-3d0/4*log(s/thmu2))
     & +born/pi*alpha*qdn**2*(3d0/4-3d0/4*log(s/thmu2))
     & +log(1-mtp**2/s)*born/pi*alpha*qdn**2*(1)
     & +log(1-mtp**2/s)**2*born/pi*alpha*qdn*qtp*(7d0/4)
     & +log(1d0/mdn**2*s)*born/pi*alpha*qdn**2*(3d0/4)
     & +log(1d0/mdn**2*s)*log(1-mtp**2/s)*born/pi*alpha*qdn*qtp*(1)
     & +log(1d0/mtp**2*s)*born/pi*alpha*qtp**2*(3d0/4+1d0/(s-mtp**2)*mtp
     & **2)
     & +log(1d0/mtp**2*s)*log(1-mtp**2/s)*born/pi*alpha*qdn*qtp*(1d0/2)
     & +log(4*omega**2/s)*born/pi*alpha*qtp**2*(-1d0/2)
     & +log(4*omega**2/s)*born/pi*alpha*qdn**2*(-1d0/2)
     & +log(4*omega**2/s)*log(1-mtp**2/s)*born/pi*alpha*qdn*qtp*(-1)
     & +log(4*omega**2/s)*log(1d0/mdn**2*s)*born/pi*alpha*qdn*qtp*(-1d0/
     & 2)
     & +log(4*omega**2/s)*log(1d0/mtp**2*s)*born/pi*alpha*qdn*qtp*(-1d0/
     & 2)
     & +ddilog(1-mtp**2/s)*born/pi*alpha*qdn*qtp*(1d0/2)
     & +ddilog(-1d0/(s-mtp**2)*mtp**2)*born/pi*alpha*qdn*qtp*(1d0/2)
     & +ddilog(1/(s-mtp**2)*s)*born/pi*alpha*qdn*qtp*(-1)
     & +ddilog(1d0)*born/pi*alpha*qdn*qtp*(-3d0/2)
     ...
     & +log(1-mbt**2/s)**2*born/pi*alpha*qup*qtp*(-1)
     & +log(1-mbt**2/s)**2*born/pi*alpha*qup*qdn*(-1)
     & +log(1-mtp**2/s)**2*born/pi*alpha*qdn*qbt*(-1)
     & +log(1-mtp**2/s)**2*born/pi*alpha*qup*qdn*(-1)
     & +log(1d0/(-u+mbt**2)*mbt**2)*log(1d0/mbt**2*s)*born/pi*alpha*qdn*
     & qbt*(-1)
     & +log(1d0/(-u+mbt**2)*mbt**2)*log(4*omega**2/s)*born/pi*alpha*qdn*
     & qbt*(-1)
     & +log(1d0/mup**2*s)*log(1-mbt**2/s)*born/pi*alpha*qup*qtp*(-1)
     & +log(1d0/mup**2*s)*log(1-mbt**2/s)*born/pi*alpha*qup*qdn*(-1)
     & +log(1d0/mdn**2*s)*log(1-mtp**2/s)*born/pi*alpha*qdn*qbt*(-1)
     & +log(1d0/mdn**2*s)*log(1-mtp**2/s)*born/pi*alpha*qup*qdn*(-1)
     & +log(1d0/mtp**2*s)**2*born/pi*alpha*qup*qtp*(1d0/2)
     & +log(1d0/mbt**2*s)**2*born/pi*alpha*qdn*qbt*(-1d0/2)
     & +log(1d0/s*(-u+mtp**2))**2*born/pi*alpha*qup*qtp*(-1d0/2)
     ...
     & )
     & +log(4*omega**2/s)*log(1d0/mbt**2*s)*born/pi*alpha*qdn*qbt*(-1d0/
     & 2)
     & +log(4*omega**2/s)*log(1d0/s*(-u+mtp**2))*born/pi*alpha*qup*qtp*(
     & 1)
     & +log(4*omega**2/s)*log(-1d0/s*t)*born/pi*alpha*qup*qdn*(1)
     & +log((-u+mbt**2)/mbt**2)**2*born/pi*alpha*qdn*qbt*(-1d0/2)
     ...
     & +ddilog(1-1/(-u+mbt**2)*(s-mtp**2))*born/pi*alpha*qdn*qbt*(-1)
     & +ddilog(1-1/(-u+mbt**2)*(s-mtp**2)*mbt**2/s)*born/pi*alpha*qdn*qb
     & t*(-1)
     & +ddilog(-1d0/mtp**2*s+(s-mbt**2)/(mtp**2-u))*born/pi*alpha*qup*qt
     & p*(1)
     & +ddilog(-mtp**2/s+(s-mbt**2)/(mtp**2-u)*mtp**2/s)*born/pi*alpha*q
     & up*qtp*(1)
     & +ddilog(1/(u-mbt**2)*u)*born/pi*alpha*qdn*qbt*(1)
     & +ddilog(1/(u-mtp**2)*u)*born/pi*alpha*qup*qtp*(1)
     & +ddilog(1d0)*born/pi*alpha*qdn*qbt*(-1)
     & +ddilog(1d0)*born/pi*alpha*qup*qtp*(-1)
