...
************************************************************************
* File (cc-qcd_ha_1413_1615.f) is created on Tue Aug  9 22:38:19 MSD 2011.
************************************************************************
* This is the FORTRAN module (cc_qcd_ha_1413_1615) to calculate differential
* hard gluon radiation for the anti-dn + up -> mo^+ + mn process.
*
...
* E-mail: <sanc@jinr.ru>
************************************************************************
      subroutine cc_qcd_ha_1413_1615 (s,spr,ph4,costh4,costh5,hard)
      implicit none!
     ...
     & 2p4*mdn**2*mmo**2/s**2/spr**3+1d0/6*p2p4*p3p5*mdn**2/s**2/spr**2-
     & 1d0/6*p2p4*p3p5*mdn**2*mmo**2/s**2/spr**3)
     & +iz1dn*iz2up*theta_hard(s,spr,omega)*chiwspr*chiwsprc/pi*alpha**2
     & *alphas*(s-spr)*cf*(-1d0/3*p1p2*p1p3*p2p4/s**2/spr**2+1d0/3*p1p2*
     & p1p3*p2p4*mmo**2/s**2/spr**3+1d0/6*p1p2*p1p3*p4p5/s**2/spr**2-1d0
     & /6*p1p2*p1p3*p4p5*mmo**2/s**2/spr**3+1d0/6*p1p2*p2p4*p3p5/s**2/sp
     & r**2-1d0/6*p1p2*p2p4*p3p5*mmo**2/s**2/spr**3)
     & +iz1dn*chiwspr*chiwsprc/pi*alpha**2*alphas*(s-spr)*cf*(1d0/12*p1p
     & 3*p1p4/s**2/spr**2-1d0/12*p1p3*p1p4*mmo**2/s**2/spr**3-1d0/12*p1p
     & 3*p2p4/s**2/spr**2+1d0/12*p1p3*p2p4*mmo**2/s**2/spr**3+1d0/12*p2p
     & 4*p3p5/s**2/spr**2-1d0/12*p2p4*p3p5*mmo**2/s**2/spr**3)
     & +iz2up**2*theta_hard(s,spr,omega)*chiwspr*chiwsprc/pi*alpha**2*al
     & phas*(s-spr)*cf*(-1d0/6*p1p3*p2p4*mup**2/s**2/spr**2+1d0/6*p1p3*p
     & 2p4*mup**2*mmo**2/s**2/spr**3+1d0/6*p1p3*p4p5*mup**2/s**2/spr**2-
     & 1d0/6*p1p3*p4p5*mup**2*mmo**2/s**2/spr**3)
     & +iz2up*chiwspr*chiwsprc/pi*alpha**2*alphas*(s-spr)*cf*(-1d0/12*p1
     & p3*p2p4/s**2/spr**2+1d0/12*p1p3*p2p4*mmo**2/s**2/spr**3+1d0/12*p1
     & p3*p4p5/s**2/spr**2-1d0/12*p1p3*p4p5*mmo**2/s**2/spr**3+1d0/12*p2
     & p3*p2p4/s**2/spr**2-1d0/12*p2p3*p2p4*mmo**2/s**2/spr**3)
