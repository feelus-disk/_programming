      COMPLEX*16 FUNCTION JAW_tbud_subsub1(Qs,Ps,m12,m22,m32)
*--------------------------------------------------------
      IMPLICIT NONE!
      ...
      COMPLEX*16 P,XSPENZ,MASTER_TBUD_SUBSUB1,JAW0,JAW1,JAW2
...
* The second part:
     ...
     &     +MASTER_TBUD_SUBSUB1(zlm,zlp)
     &     -MASTER_TBUD_SUBSUB1(DCMPLX(1d0,0d0),zlp)
     &     -CDLOG(-(Ps-ieps)/Ps)*CDLOG(1d0-1d0/zlm)
     &     +1d0/2*CDLOG(1d0-1d0/zlm)**2
     &     +MASTER_TBUD_SUBSUB1(zlp,zlm)
     &     -MASTER_TBUD_SUBSUB1(DCMPLX(1d0,0d0),zlm)
     &   -2*XSPENZ(1d0/(1d0-zlm))
     &   +(2*CDLOG((rm22-rm32+ieps)/Ps)+DLOG(-Ps/rm32)
     &    )*CDLOG(-rm32*rm22/(rm22-m32)**2)
     &              )
      JAW_tbud_subsub1=JAW0+JAW1+JAW2
      RETURN
      END

      COMPLEX*16 FUNCTION MASTER_TBUD_SUBSUB1(ayl,ayd)
*----------------------------------------
