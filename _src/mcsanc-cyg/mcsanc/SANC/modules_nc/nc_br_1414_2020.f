...
************************************************************************
* File (nc_br_1414_2020.f) is created on Wed Apr 18 13:21:48 MSK 2012.
************************************************************************
* This is the FORTRAN module (nc_bo_1414_2020) to calculate EW
* integrated Born cross section for the anti-dn + dn -> ta^+ + ta^- process.
*
...
* E-mail: <sanc@jinr.ru>
************************************************************************
      subroutine nc_bo_1414_2020 (s,born)
      implicit none!
...
************************************************************************
* This is the FORTRAN module (nc_br_1414_2020) to calculate EW Born
...
* E-mail: <sanc@jinr.ru>
************************************************************************
      subroutine nc_br_1414_2020 (qs,ts,us,born,soft,hard)
      implicit none!
      include 's2n_declare.h'
     ...
     & s*mta**2+ts**2)*qs*ts)*born/pi*alpha*qdn*qta*(2)
     & +log(1+1d0/qs/us*mta**4+2/qs*mta**2+1d0/qs*us)*log(-1/(mta**4+2*u
     & s*mta**2+us**2)*qs*us)*born/pi*alpha*qdn*qta*(-2)
     & +log(1-2/(1+betastata)/(mta**2+us)*mta**2)*born/pi*alpha*qdn*qta*
     & (-2*jintqstata)
     & +log(1-2/(1+betastata)/(mta**2+us)*mta**2)**2*born/pi*alpha*qdn*q
     & ta*(1)
     & +log(1-2/(1+betastata)/(mta**2+ts)*mta**2)*born/pi*alpha*qdn*qta*
     & (2*jintqstata)
     & +log(1-2/(1+betastata)/(mta**2+ts)*mta**2)**2*born/pi*alpha*qdn*q
     & ta*(-1)
     & +log(-1/(mta**4+2*us*mta**2+us**2)*qs*us)**2*born/pi*alpha*qdn*qt
     & a*(-1)
     & +log(-1/(mta**4+2*ts*mta**2+ts**2)*qs*ts)**2*born/pi*alpha*qdn*qt
     & a*(1)
     & +ddilog(1-2/(1+betastata)/(mta**2+us)*us)*born/pi*alpha*qdn*qta*(
     & -2)
     & +ddilog(1-2/(1+betastata)/(mta**2+ts)*ts)*born/pi*alpha*qdn*qta*(
     & 2)
     & +ddilog(-1d0/qs/ts*mta**4-2/qs*mta**2-1d0/qs*ts)*born/pi*alpha*qd
     & n*qta*(-2)
     & +ddilog(-1d0/qs/us*mta**4-2/qs*mta**2-1d0/qs*us)*born/pi*alpha*qd
     & n*qta*(2)
     ...
     & mta**2+betastata*us)*betastata*us)*born/pi*alpha*qdn*qta*(2)
     ...
     & mta**2+betastata*ts)*betastata*ts)*born/pi*alpha*qdn*qta*(-2)
...
************************************************************************
* This is the FORTRAN module (nc_ha_1414_2020_1spr) to calculate hard
* photon Bremsstrahlung for the anti-dn + dn -> ta^+ + ta^- process.
*
* It is produced by s2n package of SANC (v1.10) Project.
* Copyright (C), SANC Project Team, 2002-2012.
*
...
* E-mail: <sanc@jinr.ru>
************************************************************************
      subroutine nc_ha_1414_2020_1spr (s,spr,hard)
      implicit none!
      include 's2n_declare.h'

      real*8 kappaz,coef,sqs
      real*8 betasdndn,betasprtata
      real*8 jintsdndn,jintsprtata,ln1sdn,ln1bsprta
     ...
     & ga)*(8d0/9*mta**2/s**3)
     & +v0spr*betasprtata*(s**2+spr**2)*alpha**3*qdn**2/spr**2/(s-spr)*t
     & heta_hard(s,spr,omega)*(-4d0/9*ln1sdn*mta**2/s**2)
     & +v0spr*betasprtata*(s**2+spr**2)*alpha**3*qdn**2/spr/(s-spr)*thet
     & a_hard(s,spr,omega)*(4d0/9*ln1sdn/s**2)
     & +a2sspr*(s+spr)**2*alpha**3*qdn*qta/spr/(s-spr)*theta_hard(s,spr,
     & omega)*(-2d0/3*jintsprtata*mta**2/s**3)
     & +a2sspr*betasprtata*(s+spr)*alpha**3*qdn*qta/(s-spr)*theta_hard(s
     & ,spr,omega)*(-2d0/3/s**2)
     ...
     & ga)*(-8d0/3*mta**2/s**3)
     & +vaspr*betasprtata*(s**2+spr**2)*alpha**3*qdn**2/spr**2/(s-spr)*t
     & heta_hard(s,spr,omega)*(4d0/3*ln1sdn*mta**2/s**2)
     & +a1sspr*(s+spr)*alpha**3*qdn**2*qta**2/spr*(-2d0/3*jintsprtata*mt
     & a**2/s**3)
