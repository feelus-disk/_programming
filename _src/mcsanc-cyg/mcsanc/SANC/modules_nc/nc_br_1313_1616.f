...
************************************************************************
* File (nc_br_1313_1616.f) is created on Wed Apr 18 13:16:16 MSK 2012.
************************************************************************
* This is the FORTRAN module (nc_bo_1313_1616) to calculate EW
* integrated Born cross section for the anti-up + up -> mo^+ + mo^- process.
*
...
* E-mail: <sanc@jinr.ru>
************************************************************************
      subroutine nc_bo_1313_1616 (s,born)
      implicit none!
...
************************************************************************
* This is the FORTRAN module (nc_br_1313_1616) to calculate EW Born
...
* E-mail: <sanc@jinr.ru>
************************************************************************
      subroutine nc_br_1313_1616 (qs,ts,us,born,soft,hard)
      implicit none!
      include 's2n_declare.h'
     ...
     & s*mmo**2+ts**2)*qs*ts)*born/pi*alpha*qup*qmo*(2)
     & +log(1+1d0/qs/us*mmo**4+2/qs*mmo**2+1d0/qs*us)*log(-1/(mmo**4+2*u
     & s*mmo**2+us**2)*qs*us)*born/pi*alpha*qup*qmo*(-2)
     & +log(1-2/(1+betasmomo)/(mmo**2+us)*mmo**2)*born/pi*alpha*qup*qmo*
     & (-2*jintqsmomo)
     & +log(1-2/(1+betasmomo)/(mmo**2+us)*mmo**2)**2*born/pi*alpha*qup*q
     & mo*(1)
     & +log(1-2/(1+betasmomo)/(mmo**2+ts)*mmo**2)*born/pi*alpha*qup*qmo*
     & (2*jintqsmomo)
     & +log(1-2/(1+betasmomo)/(mmo**2+ts)*mmo**2)**2*born/pi*alpha*qup*q
     & mo*(-1)
     & +log(-1/(mmo**4+2*us*mmo**2+us**2)*qs*us)**2*born/pi*alpha*qup*qm
     & o*(-1)
     & +log(-1/(mmo**4+2*ts*mmo**2+ts**2)*qs*ts)**2*born/pi*alpha*qup*qm
     & o*(1)
     & +ddilog(1-2/(1+betasmomo)/(mmo**2+us)*us)*born/pi*alpha*qup*qmo*(
     & -2)
     & +ddilog(1-2/(1+betasmomo)/(mmo**2+ts)*ts)*born/pi*alpha*qup*qmo*(
     & 2)
     & +ddilog(-1d0/qs/ts*mmo**4-2/qs*mmo**2-1d0/qs*ts)*born/pi*alpha*qu
     & p*qmo*(-2)
     & +ddilog(-1d0/qs/us*mmo**4-2/qs*mmo**2-1d0/qs*us)*born/pi*alpha*qu
     & p*qmo*(2)
     ...
     & mmo**2+betasmomo*us)*betasmomo*us)*born/pi*alpha*qup*qmo*(2)
     ...
     & mmo**2+betasmomo*ts)*betasmomo*ts)*born/pi*alpha*qup*qmo*(-2)
...
************************************************************************
* This is the FORTRAN module (nc_ha_1313_1616_1spr) to calculate hard
* photon Bremsstrahlung for the anti-up + up -> mo^+ + mo^- process.
*
* It is produced by s2n package of SANC (v1.10) Project.
* Copyright (C), SANC Project Team, 2002-2012.
*
...
* E-mail: <sanc@jinr.ru>
************************************************************************
      subroutine nc_ha_1313_1616_1spr (s,spr,hard)
      implicit none!
      include 's2n_declare.h'

      real*8 kappaz,coef,sqs
      real*8 betasupup,betasprmomo
      real*8 jintsupup,jintsprmomo,ln1sup,ln1bsprmo
     ...
     & ga)*(8d0/9*mmo**2/s**3)
     & +v0spr*betasprmomo*(s**2+spr**2)*alpha**3*qup**2/spr**2/(s-spr)*t
     & heta_hard(s,spr,omega)*(-4d0/9*ln1sup*mmo**2/s**2)
     & +v0spr*betasprmomo*(s**2+spr**2)*alpha**3*qup**2/spr/(s-spr)*thet
     & a_hard(s,spr,omega)*(4d0/9*ln1sup/s**2)
     & +a2sspr*(s+spr)**2*alpha**3*qup*qmo/spr/(s-spr)*theta_hard(s,spr,
     & omega)*(-2d0/3*jintsprmomo*mmo**2/s**3)
     & +a2sspr*betasprmomo*(s+spr)*alpha**3*qup*qmo/(s-spr)*theta_hard(s
     & ,spr,omega)*(-2d0/3/s**2)
     ...
     & ga)*(-8d0/3*mmo**2/s**3)
     & +vaspr*betasprmomo*(s**2+spr**2)*alpha**3*qup**2/spr**2/(s-spr)*t
     & heta_hard(s,spr,omega)*(4d0/3*ln1sup*mmo**2/s**2)
     & +a1sspr*(s+spr)*alpha**3*qup**2*qmo**2/spr*(-2d0/3*jintsprmomo*mm
     & o**2/s**3)
