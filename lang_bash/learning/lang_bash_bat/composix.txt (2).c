...
• false<cu>		возвратить значение 1, 
• true<cu>		возвратить значение 0
...
{• pwd<ba><cu>	(print working directory) Выдает полное имя текущего каталога.
	pwd [-LP]
...
{• ls<cu>		печатает в стандартный вывод содержимое директорий.
...
{• ln<cu>		устанавливающая связь между файлом и именем файла. (hard link, symbolic linc)
ln dest link
по умолчанию жесткую
--symbolic - символьную
}
• cp<cu>		копирование файлов из одного в другие каталоги (возможно, с другой файловой системой)
 cpdir<??>	
• mv<cu>		для перемещения или переименования файлов.
• rm<cu> 		для удаления файлов из файловой системы
• mkdir<cu>		команда для создания новых каталогов
• rmdir<cu>		удаляет каталог из файловой системы
...
. mknod<cu>		Создаёт специальные файлы
...
{• echo<ba>	Выдает аргументы через пробел, дополняя в конце символом новой строки.
	echo [-neE] [аргумент ...]
...
• cat<cu>		утилита UNIX, выводящая последовательно указанные файлы (или устройства), таким образом, объединяя их в единый поток
...
{==== ====
• dd<cu>		программа UNIX, предназначенная как для копирования, так и для конвертации файлов
...
{== прочее общее администрирование системы ==
	...
	диски)
{=== терминал ===
• stty<cu>		Изменяет настройки терминала
• tty<cu>		выводящая имя терминала, соединённого со стандартным вводом
}
{=== сигналы и процессы ===
{• kill<ba>		Посылает указанный по имени или номеру сигнал процессам с указанными идентификаторами
...
• uname<cu>		выводящая информацию о системе.
...
• date<cu>		утилита Unix для работы с системными часами. 
...
su
sudo
• chmod<cu>		программа для изменения прав доступа к файлам и директориям
• chgrp<cu>		утилита UNIX. Может использоваться непривелигированными пользователями для изменения группы владельца файлов
• chown<cu>		UNIX?утилита, изменяющая владельца и/или группу для указанных файлов
...
• df<cu>		показывает список всех файловых систем по именам устройств, сообщает их размер, занятое и свободное пространство и точки монтирования.
• fuser 		идентифицирующая процессы, которые используют указанные файлы или сокеты.
. sync<cu> 		Записывает на диск буферы файловых систем
}
}
{== архиваторы ==
...
vcg				просмотрщик rational fortran, представляющий текстовый файл в виде графа. в GCC есть опция -dv для генерирования данных для оптимизации в формате, поддерживаемом vcg.
{=== автотулс ===
