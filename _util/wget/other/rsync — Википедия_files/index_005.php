...
window.pg || $(document).ready( function($) {
    ...
    if ( wgCanonicalNamespace === '' || wgCanonicalNamespace === 'Project' || wgCanonicalNamespace === 'Help' ) {
        ...
        if( settings[0] == 0 ) {
            ...
            footer.append($("<li>").append($("<a>").text("Включить всплывающие подсказки").attr("href","javascript:(function(){})()").click(function(){toggleRT(1)})));
            return;
        }
            ...
            function openSettingsMenu(){
                    if( settingsMenu ) {
                            ...
                            settingsMenu = $("<form>").append(
                                    $("<button>").css("width","100%").text("Отключить всплывающие подсказки").button().click(function(){toggleRT(0)}),
                                    $("<br>"),
                                    $("<small>").text("После отключения всплывающие подсказки могут быть повторно включены при помощи ссылки в подвале страницы."),
                                    $("<hr>"),
                                    $("<label>").text("Задержка перед появлением подсказки (в миллисекундах): ").append($("<input>").attr({"type":"number","value":settings[1],step:50,min:0,max:5000})),
                                    $("<br>"),
                                    $("<span>").text("Подсказка появляется при"),
                                    $("<label>").append(
                            ...
                            ).submit(function(e){e.preventDefault()}).dialog({modal:true,width:500,title:"Настройки всплывающих подсказок",buttons:{"Сохранить настройки":function(){
