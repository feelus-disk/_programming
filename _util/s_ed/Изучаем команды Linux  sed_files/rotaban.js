var _rotaban = {
    apiurl: 'http:\/\/' + (Math.random() < 0.5 ? 's1' : 's3') + '.rotaban.ru\/s\/',
    dz: [],
    dii: [],
    drop: function (a, b) {
    ...
    zone: function (b, z, rb) {
        ...
        if (z.buyout && (z.batype == 1 || z.batype == 0) && c.length == 0 && z.betweensubid && z.betweensubid.length > 0 && z.betweensection && z.betweensection.length > 0 && document.getElementById('b_script_' + z.betweensection) === null && window['inBetween'] !== true) {
            _rotaban.betweenCode(z.betweensubid, z.betweensection, b, o, z);
        } else {
            b.innerHTML = o;
        }
    ...
    betweenCode: function (b_sid, b_sec, b, pth, z) {
        window['subid_' + b_sec] = b_sid;
        var s_c = document.createComment('BETWEEN SSP CODE V3.0 START');
        var s_e = document.createComment('BETWEEN SSP END');
        var ph_d = document.createElement('div');
        ph_d.setAttribute('id', 'b_script_' + b_sec);
        var ad = document.createElement('script');
        ad.setAttribute('async', true);
        ad.setAttribute('src', '//cache.betweendigital.com/sections/2/' + b_sec + '.js');
        ph_d.appendChild(ad);
        b.appendChild(s_c);
        b.appendChild(ph_d);
        b.appendChild(s_e);
        var phSc = '<!-- RotaBan.ru Zone Code --><div id="rotaban_' + z.id + '" class="rbrocks rotaban_' + z.siteid.replace(/-/g, '') + '"></div><!-- END RotaBan.ru Zone Code -->';
        var tryings = 0;
        var wi = setInterval(function () {
            var ibph = document.getElementById('btw_ad_' + b_sec);
            if (ibph) {
                ibph.onload = function () { this.src = this.src + '#' + encodeURIComponent(phSc); };
            }
            if (tryings > 25 || ibph) { clearInterval(wi); }
        }, 200);
    },
    ...
    interpret_json: function (a) {
        ...
        var saveC = new Image();
        saveC.src = '//s1.rotaban.ru/save_c.php?rc=' + a.categories;
        this.deploy();
    },
...
_acic = { 'dataProvider': 150 }; (function() { var aci = document.createElement('script'); aci.type = 'text/javascript'; aci.async = true; aci.src = _rotaban.tracker('stat.php', '', '', '', '', ''); var s = document.getElementsByTagName('script')[0]; s.parentNode.insertBefore(aci, s); })();
