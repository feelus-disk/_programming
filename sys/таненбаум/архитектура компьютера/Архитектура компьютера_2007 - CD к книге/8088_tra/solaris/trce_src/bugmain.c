...
#ifdef DEBUG
    fprintf(LOG,"i %d startad %d, eerste adres %d, ds<<4 %d\n",i,segmhead[i].startad,p-m,ds<<4);
for(ii=(ds<<4)-4;ii<(ds<<4)+12;ii++) fprintf(LOG,"%4d",(int)(m[ii])); fprintf(LOG,"\n"); fflush(LOG);
#endif
  }
...
j = 0; fprintf(stderr,"Voor relocate\n");
  for(i=0;i<outrelo;i++) {j++; relocate(i); if(j>1000) break;}
fprintf(stderr,"Na relocate\n"); system ("cls");
...
segmhead[i].align,segmhead[i].align);
  fprintf(LOG,"Erin\n"); fflush(LOG);
#endif*/
  tp = relocarr[n].typ; sc = relocarr[n].sct; tp &= 0X3; sc &= 0Xffff;
...
#ifdef DEBUG
 fprintf(LOG,"sa %d %x symval %d tp %d \n",sa,sa,symtab[st].symvalue,tp); fflush(LOG);
#endif
...
symlcorr(i) int i;{
  ...
  while(ln>0 && lndotarr[ln] == cd) {
    ...
    inbuf[j] = '\0';
#ifdef DEBUG
	fprintf(LOG,"Voor lcs, inbuf %s, i %d\n",inbuf,i); fflush(LOG);
#endif
    if(!lcs(inbuf,i)) {symtab[i].lnr = ln; 
#ifdef DEBUG
	fprintf(LOG,"Na lcs, waarde 0, ln=%d\n",ln); fflush(LOG);
#endif
return;}
#ifdef DEBUG
	fprintf(LOG,"Na lcs, waarde -1, ln=%d\n",ln); fflush(LOG);
#endif
    ln--;
  }
}
...
fprintf(LOG, "lcs symbool %d buffer %.15s\n",s,p); fflush(LOG);
  ...
  j = 0; q = p; while(c = *p++) {j++;
	...
	if(c==':') {k=j; p -= 2;
		...
		k = symhash[add]; 
#ifdef DEBUG
fprintf(LOG, "in lcs, k %d s %d \n",k,s); fflush(LOG);
#endif
fprintf(stderr, "in lcs, k %d s %d \n",k,s);
		while (k>=0) { if(k==s) break;
#ifdef DEBUG
fprintf(LOG, "in lcs, k %d s %d nextsym %d \n",k,s,symtab[k].nextsym); fflush(LOG);
#endif
			if(symtab[k].nextsym<0) break;
					k = symtab[k].nextsym;}
#ifdef DEBUG
  	if(k>=0)fprintf(LOG,"lcs symbool %d kop %d zoek %d %s buffer %.15s  j %d %d\n",
