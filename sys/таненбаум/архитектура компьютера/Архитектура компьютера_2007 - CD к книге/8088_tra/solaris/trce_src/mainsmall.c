...
"%6d%5x startad | %6d%5x lengte | %6d%5x fstart | %6d%5x flen | %6d%5x align\n",
...
#endif
  if(j>32000) {fprintf(stderr,"Insufficient amount of memory %x\n",j); exit(1);}
  }
  for(i=0;i<sections;i++) {
    ...
    if(!i) { ss = es = ds = (segmhead[i].lengte + 15)>>4;
		bp = maxsp = sp = 0x8000 - (ss<<4);}
#ifdef DEBUG
...
fprintf(LOG,"Tot hier dotlinarr %d PC %d\n",dotlnarr[(PC)-1],(PC)-1);
 ...
 for(i=0;i<7;i++) sprintf(window[17+i],"%s",datadarr[i]);
  l = prdepth; j= (maxsp-sp > 18) ? sp : maxsp-18; 	p = m+j+(ss<<4);
