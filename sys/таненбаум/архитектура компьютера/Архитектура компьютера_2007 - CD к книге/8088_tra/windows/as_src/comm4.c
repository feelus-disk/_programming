...
void stop(int dummy) {
/*fprintf(stderr,"Temporary file %s will be unlinked\n",temppath);*/
	unlink(temppath);
#if DEBUG < 2
#ifdef LISTING
/*fprintf(stderr,"Temporary file %s will be unlinked %s\n",listpath);*/
	unlink(listpath);
#endif
#endif
	fclose(input);
...
char **argv;
{
	...
	register i;
	static char sigs[] = {
		SIGHUP&255, SIGINT&255, SIGQUIT&255, SIGTERM&255, 0
	};
   ...
   if((ascptr= fopen("ASCIIFIL.$$$","wb")) == NULL){
	...
	if (((inptr = fopen(p, "rw")) == NULL) &&
	    ((inptr = fopen(ffnm, "rw")) == NULL) &&
	     ((inptr = fopen(filsnm, "rw")) == NULL) &&
	      ((inptr = fopen(filcapnm, "rw")) == NULL))
	...
	if (rflag)
		sflag |= SYM_SCT;
#endif /* RELOCATION */
/*fprintf(stderr,"Ik ben hier filechar voor pass 1\n");*/
	pass_1(argc,argv);
/*fprintf(stderr,"Ik ben hier filechar na pass 1\n");*/
/*#ifdef THREE_PASS*/
...
/*#ifdef RELOCATION
	...
	pass_1(argc,argv); */
#ifdef THREE_PASS
/*fprintf(stderr,"Ik ben hier filechar voor pass 2\n");*/
	pass_23(PASS_2);
#endif
/*fprintf(stderr,"Ik ben hier filechar voor pass 3\n");*/
	pass_23(PASS_3);
/*fprintf(stderr,"Ik ben hier filechar na pass 3\n");*/
/*fprintf(stderr,"Ik ben hier filechar %s\n",filechar);*/
fclose(inptr); fclose(uitptr); unlink(filechar); /*fclose(inptr);fclose(uitptr);
 		if ((inptr = fopen("ASCIIFIL.$$$", "rb")) == NULL)
 			fatal("can't open ASCIIFIL.$$$");
 		if ((uitptr = fopen(filechar, "wb")) == NULL)
 			fatal("can't open %s", filechar);
 	while(1){cpychar=getc(inptr); if(cpychar == EOF) break; putc(cpychar,stderr);putc(cpychar,uitptr);}

 	fclose(inptr); fclose(uitptr);*/
rename("ASCIIFIL.$$$",filechar);
	wr_close();
	/*system("move ASCIIFIL.$$$ SCIIFIL.$$$");*/
	stop(0);
}
...
char **argv;
{
	...
	tempfile = fftemp(temppath, "asTXXXXX.$$$");
#ifdef LISTING
	listmode = dflag;
	if (listmode & 0440)
		listfile = fftemp(listpath, "asLXXXXX.$$$");
#endif
	...
	/*while (--argc >= 0)*/ {
		...
		if (p == 0)
			continue; */
	sprintf(filechar,"%s.$",projname);
	if ((input = fopen(filechar, "rw")) == NULL)
