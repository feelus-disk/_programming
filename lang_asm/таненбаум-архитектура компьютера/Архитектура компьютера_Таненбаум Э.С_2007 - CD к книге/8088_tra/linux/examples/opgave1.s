...
inplmin: !+ of - tegengekomen als karretje 
	mov bp, sp
	mov ax, 2(bp)
	CMP AX,'=' 
	JE 1f 
...
inmldvrs:	!* of : tegengekomen als karretje 
	mov bp, sp
	mov ax, 2(bp)
	CMP AX,'*' 
	JE 1f 
...
inneg:	!~ tegengekomen als karretje 
	CMP  (nxtbuf),'~' 
	JNE  1f		!bij wel equal: dubbele negatie, beide tekens overslaan. 
	CALL getcbuf 
	RET 
1:	CMP  (nxtbuf),'=' 
	JNE  2f		!bij wel equal: dubbele negatie, beide tekens overslaan. 
	CALL inermes 
	RET 
2:	POP  AX 
	CMPB AL,'~' 
...
inhs:	!) tegengekomen als karretje 
	mov bp, sp
	mov ax, 2(bp)
	CMP AX,'=' 
	JE 1f 
	CMP AX,'(' 
	JE 2f 
	PUSH AX !herstel de stack 
	CALL incutin 
	pop ax
	RET 
1:	CALL inermes 
	RET 
2:	PUSH AX !herstel de stack 
	CALL indestr 
	RET 
	 
	 
inpass: !blabla stuur het karretje meteen door 
	MOVB (BX),DL !DX bevat de byte die ingelezen is door getcbyte 
	inc bx 
	RET 
 
incutin: !stuur een karretje van Texas naar California 
	mov bp, sp
	mov dx, 2(bp)
	MOVB (BX),DL 
	INC BX 
	...
	DEC AX 
	MOV (pinbuf),AX 
	!DEC pinbuf !zorg ervoor dat het karretje vooraan de lijn weer opnieuw gelezen wordt 
	RET 
 
indestr: !hijack karretjes 
	!ADD SP,2 !verwijder het karretje van de stack 
	!het andere karretje wordt automatisch genegeerd doordat getcbuf naar het volgende karakter gaat. 
	RET 
	 
inerror: 
	MOVB (inputb),'\0' 
	MOV (pinbuf),inputb !zet pointer weer aan begin van inputb 
	PUSH inermes	! Print error message 
	PUSH (_printf) 
	SYS 
	ADD  SP,4 
	RET 
 
instack: !stuur het karretje naar Texas 
	PUSH DX 
	RET 
	 
inequ:	!= tegengekomen als karretje 
	mov bp, sp
	mov ax, 2(bp)
	CMP AX,'=' 
	JE 1f 
...
! <b>De conversie tabel voor infix to postfix</b> 
INTABLE: .WORD	snop,	snop,	snop,	snop,	snop,	snop,	snop,	snop  
	!	cntrl-0 cntrl-A cntrl-B cntrl-C cntrl-D cntrl-E cntrl-F bell 
	.WORD	snop,	inpass,	inpass,	inpass,	inpass,	inpass,	snop,	snop 
	!	back	hortab	newline	vertab	newpag	carret	shout	shin 
	.WORD	snop,	snop,	snop,	snop,	snop,	snop,	snop,	snop 
	.WORD	snop,	snop,	snop,	snop,	snop,	snop,	snop,	snop 
	.WORD	inpass,	snop,	snop,	snop,	snop,	inmldvrs,	snop,	snop 
	!	space	!	"	#	$	%	&	' 
	.WORD	instack,	inhs,	inmldvrs,	inplmin,	snop,	inplmin,	snop,	snop 
	!	(	)	*	+	,	-	.	/ 
	.WORD	inpass,	inpass,	inpass,	inpass,	inpass,	inpass,	inpass,	inpass 
	!	0	1	2	3	4	5	6	7 
	.WORD	inpass,	inpass,	inmldvrs,	snop,	snop,	inequ,	snop,	snop 
	!	8	9	:	;	<	=	>	? 
	.WORD	snop,	inpass,	inpass,	inpass,	inpass,	inpass,	inpass,	inpass 
	!	@	A	B	C	D	E	F	G 
	.WORD	inpass,	inpass,	inpass,	inpass,	inpass,	inpass,	inpass,	inpass 
	!	H	I	J	K	L	M	N	O 
	.WORD	inpass,	inpass,	inpass,	inpass,	inpass,	inpass,	inpass,	inpass 
	!	P	Q	R	S	T	U	V	W 
	.WORD	inpass,	inpass,	inpass,	snop,	snop,	snop,	inpass,	snop 
	!	X	Y	Z	[	\	]	^	- 
	.WORD	snop,	snop,	snop,	snop,	snop,	snop,	snop,	snop 
	!	`	a	b	c	d	e	f	g 
	.WORD	snop,	snop,	snop,	snop,	snop,	snop,	snop,	snop 
	!	h	i	j	k	l	m	n	o 
	.WORD	snop,	snop,	inpass,	snop,	snop,	snop,	snop,	snop 
	!	p	q	r	s	t	u	v	w 
	.WORD	snop,	snop,	snop,	snop,	snop,	snop,	inneg,	snop 
	!	x	y	z	{	|	}	~	delete 
