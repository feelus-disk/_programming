...
volatile U8 DEL_ST_KL=0; //делитель задания интервала сброса ошибки PC-клавиатуры

volatile extern U8 st_bit; //счетчик битов в принимаемом байте от PC-клавиатуры
...
void timer1_c_irq_handler(void)
{
    ...
    //сброс возможной ошибки PC-клавиатуры
    DEL_ST_KL++;
    if (DEL_ST_KL > 200)
      {
      st_bit = 0; //обнулить счетчик битов
                  //чтобы сбросить возможную ошибку PC-клавиатуры
      DEL_ST_KL=0;
      }
