...
volatile U8 flagn_kn1=0, flagn_kn2=0, flagn_kn3=0, flagn_kn4=0, flagn_kn5=0;
//флаги удержания кнопок
static  U8 flagu_kn1=0, flagu_kn2=0, flagu_kn3=0, flagu_kn4=0, flagu_kn5=0;
//переменные накопления дребезга контактов кнопок
static  U8 dreb_kn1=0, dreb_kn2=0, dreb_kn3=0, dreb_kn4=0, dreb_kn5=0;
...
void opros_kn5()
 {
 if ( (AT91F_PIO_GetInput(AT91C_BASE_PIOA) & SW5_MASK) == 0) // нажата ли кнопка ?
        {
           if (dreb_kn5 > DREB_KN5_K)//да, прервыш ли порог накопл дребезга?
             {
                if  (flagu_kn5==1)   // да, установлен ли флаг удержания ?
                  {}                 // да, ничего не делать
                else
                  {
                  flagu_kn5=1;    // нет, установить флаг удержания
                  flagn_kn5=1;    // установить флаг нажатия
                  }
             }
           else   dreb_kn5++;     //не превышен,  продолжить накопление
        }
    else      //не нажата
        {
        flagu_kn5=0;     // обнулить флаг удержания
        dreb_kn5=0;      // обнулить счетчик накопления подавления дребезга
        }
 }
